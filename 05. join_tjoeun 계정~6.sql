...
/*
    2. 포괄조인 / 외부조인
      :두 테이블간의 JOIN시 일치하지 않는 행도 포함시켜 조회가능
       단, 반드시 LEFR . RIGHT를 지정해야 됨(기준이 되는 테이블 지정)
*/

-- 사원명, 부서명, 급여, 연봉
SELECT EMP_NAME, DEPT_CODE, DEPT_TITLE, SALART, SALATY*12 연봉
FROM EMPLOYEE, DEPARTMENT
WHERE DEPT_CODE = DEPT_ID;
-- 부서배치가 안된 사원 2 명 제외(DEPT_CODE = NULL)
