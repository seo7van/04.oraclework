...
SELECT TO_DATE(010610)FROM DUAL;    --숫자로 앞이 0일때는 오류가 남
SELECT TO_DATE('010610')FROM DUAL;  --이렇게 따옴표 붙혀주면 오류안남
...
--SELECT TO_CAAR('070407 020811','YY-MM-DD HH:MI:SS')FROM DUAL;

--SELECT TO_DATE('041030 143000','YYDDMM HHMISS')FROM DUAL;   --오류 : 오전 오후로 14시는 없어서
...
-- SELECT '1,000' + '5,000' FROM DUAL; --오류 : 숫자에 컴마가 들어가서
...
SELECT EMP_NAME, SALARY, JOB_CODE,
    DECODE(JOB_CODE,'J7',SALARY*1.1,
                    'J6',SALARY*1.15,
                    'J5',SALARY*1.2,
                         SALARY*1.05)   --'그외'는 J5이런거 안넣어줘도 됨 인상된 급여
    FROM EMPLOYEE;
...
SELECT EMP_ID, EMP_NAME     --가지고 오는거 첫번째에 적기
FROM EMPLOYEE
WHERE MOD(EMP_ID,2)=1;

-- 5. EMPLOYEE테이블에서 근무 년수가 20년 이상인 직원 정보 조회
SELECT *    --*이거는 정보 다 불러와라 ~ 이소리
FROM XMPLOYEE
WHERE MONTHS_BETWEEN(SYSDATE, HIRE_DATE)>240; --OR 20*12; 도 가능 비교연산자보다 산술 연산자가 더 우선순위라서
...
SELECT EMP_NAME, DEPT_CODE,
       EXTPACT(YEAR RFOM SYSDATE)-EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,2),'RRRR')) 나이
       FROM EMPLOYEE;
...
SELECT EMP_NAME,EMP_NO,
       ...
       SUBSTR(EMP_NO,8)뒷자리, --뒤에다~는 안넣어줘도 됨
